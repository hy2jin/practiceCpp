...
CChartAxis::CChartAxis() 
 : m_pParentCtrl(NULL), m_bIsHorizontal(true), m_bIsInverted(false), 
   ...
   m_nFontSize(115), m_strFontName(_T("NanumSquareRound Bold")), m_TextColor(0),//LEEHJ 폰트
	...
	if (m_bIsHorizontal)
	{
		...
		if (!m_bIsSecondary)
		{
			...
			if (bLabelOnAxis)
			{
				//// LIMJI _TEST
				// 가로 문자 테스트
				int RowCnt = _ttoi(strBuffer.c_str());
				int TextCount = m_RowText.GetCount();
				if (TextCount > RowCnt && TextCount > 0)
				{
					//LEEHJ 가운데 정렬
					CString strLabelText = m_RowText[RowCnt].GetString();
					int dist = strLabelText.GetLength()/*TextSize.cx*/ / 2;
					if (IsCellCenter()) {
						int len = strBuffer.length();
						dist *= TextSize.cx / len;
					}
					pDC->ExtTextOut(lLabelPos - dist, m_AxisRect.top + 5,
						ETO_CLIPPED | ETO_OPAQUE, NULL, m_RowText[RowCnt].GetBuffer(), NULL);
...
void CChartAxis::SetAutomaticMode(EAxisAutoModes AutoMode)
{
	...
	//if (RefreshAutoAxis()) m_pParentCtrl->RefreshCtrl(); //LIMJI 작업끝나고 Refresh 해주기 230728
}
...
void CChartAxis::SetMinMax(double Minimum, double Maximum)
{
	...
	//RefreshScrollBar();//LIMJI 작업끝나고 Refresh 해주기 230728
	//m_pParentCtrl->RefreshCtrl();//LIMJI 작업끝나고 Refresh 해주기 230728
}
