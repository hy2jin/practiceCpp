...
#pragma region GDI+
	...
	if (Gdiplus::GdiplusStartup(&m_GDI_T, &m_GDI_S, NULL) != Gdiplus::Ok){
		TRACE(_T("GDI+¿¡·¯\n"));
		//return 0;
	}
...
#pragma region GDI+
	...
	Gdiplus::GdiplusShutdown(m_GDI_T);
...
void CChartLineSerie::DrawAll(CDC *pDC)
{
	...
	{	
		if (uLast-uFirst >= 1)
		{
			...
			///LIMJI GDI+ test
			CDC MemDC;
			BOOL bRes = MemDC.CreateCompatibleDC(pDC);
			if (bRes != NULL){
				CBitmap MemBMP; 
				bRes = MemBMP.CreateCompatibleBitmap(pDC, m_PlottingRect.Width(), m_PlottingRect.Height());
				if (bRes != NULL){
					CBitmap *pOldBitmap = MemDC.SelectObject(&MemBMP);
					if (pOldBitmap != NULL){
						MemDC.BitBlt(0, 0, m_PlottingRect.Width(), m_PlottingRect.Height(), pDC, m_PlottingRect.left, m_PlottingRect.top, SRCCOPY);
						Gdiplus::Graphics GDC(MemDC.m_hDC);
						
						GDC.SetSmoothingMode(Gdiplus::SmoothingModeAntiAlias);
						int nBitmaskR = 0x0000ff;  //255
						int nBitmaskG = 0x00ff00;  //255
						int nBitmaskB = 0xff0000;  //255
						int SerieR = (m_SerieColor & nBitmaskR);
						int SerieG = (m_SerieColor & nBitmaskG)>>8;
						int SerieB = (m_SerieColor & nBitmaskB)>>16;

						int ShadowR = (m_ShadowColor &nBitmaskR);
						int ShadowG = (m_ShadowColor &nBitmaskG);
						int ShadowB = (m_ShadowColor &nBitmaskB);

						Gdiplus::Pen pen_Shadow(Gdiplus::Color(255,(BYTE)ShadowR, (BYTE)ShadowG, (BYTE)ShadowB), m_iLineWidth);
						Gdiplus::Pen pen_New(Gdiplus::Color(255,(BYTE)SerieR, (BYTE)SerieG, (BYTE)SerieB), m_iLineWidth);
						if (m_bShadow)
						{							
							//GDC.Polyline(pShadow, pointsCount);
							Gdiplus::Point *m_SPoint = NULL;
							m_SPoint = new Gdiplus::Point[pointsCount];
							for (int i = 0; i < pointsCount; i++)
							{
								m_SPoint[i] = Gdiplus::Point(pShadow[i].x , pShadow[i].y);
							}

							GDC.DrawLines(&pen_Shadow, m_SPoint, pointsCount);
							/*GDC.DrawLines(&pen_Shadow, pShadow, pointsCount);*/
							delete[] m_SPoint;
						}

						Gdiplus::Point *m_Point = NULL;
						m_Point = new Gdiplus::Point[pointsCount];
						for (int i = 0; i < pointsCount; i++)
						{
							Gdiplus::Point m_Point1 = Gdiplus::Point(pPoints[i].x - m_PlottingRect.left, pPoints[i].y - m_PlottingRect.top);
							m_Point[i] = m_Point1;
						}

						GDC.DrawLines(&pen_New, m_Point, pointsCount);


						//GDC.DrawLine(&pen_New, Gdiplus::Point(10, 50), Gdiplus::Point(100, 10));

							
					
						delete[] m_Point;

						pDC->BitBlt(m_PlottingRect.left, m_PlottingRect.top, m_PlottingRect.right, m_PlottingRect.bottom, &MemDC, 0, 0, SRCCOPY);
						MemDC.SelectObject(pOldBitmap);
						GDC.ReleaseHDC(pDC->m_hDC);
					}
					DeleteObject(MemBMP);
				}
				DeleteDC(MemDC);
