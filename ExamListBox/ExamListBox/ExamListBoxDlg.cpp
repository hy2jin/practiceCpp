...
// ExamListBoxDlg.cpp : 구현 파일
//

#include "stdafx.h"
#include "ExamListBox.h"
#include "ExamListBoxDlg.h"
...
// CExamListBoxDlg 대화 상자



CExamListBoxDlg::CExamListBoxDlg(CWnd* pParent /*=NULL*/)
	: CDialogEx(CExamListBoxDlg::IDD, pParent)
{
	m_hIcon = AfxGetApp()->LoadIcon(IDR_MAINFRAME);
}

void CExamListBoxDlg::DoDataExchange(CDataExchange* pDX)
{
	CDialogEx::DoDataExchange(pDX);
	DDX_Control(pDX, IDC_CHAT_LIST, m_chat_list);
}

BEGIN_MESSAGE_MAP(CExamListBoxDlg, CDialogEx)
	ON_WM_SYSCOMMAND()
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
...
// CExamListBoxDlg 메시지 처리기

BOOL CExamListBoxDlg::OnInitDialog()
{
...
void CExamListBoxDlg::OnSysCommand(UINT nID, LPARAM lParam)
{
...
//  이 함수를 호출합니다.
HCURSOR CExamListBoxDlg::OnQueryDragIcon()
{
...
void CExamListBoxDlg::OnBnClickedInputButton()
{
	while (m_chat_list.GetCount() > 20)		// 채팅이 20개 이상이면
	{
		m_chat_list.DeleteString(0);		// 제일 위에 있는 문자열 삭제
	}
...
void CExamListBoxDlg::OnLbnSelchangeChatList()
{
	CString str;
	int index = m_chat_list.GetCurSel();	// 아무것도 선택하지 않았으면 -1
	if (index != LB_ERR)
	{
